    ...
    void SpawnGrid()
    {
        for (int x = 0; x < gridX; x++)
        {
            for (int z = 0; z < gridZ; z++)
            {
                ...
                clone.transform.localPosition = new Vector3(clone.transform.localPosition.x + clone.GetComponent<BuildingsNum>().buildwidth, 
                    clone.transform.localPosition.y + Random.Range(1, 11), clone.transform.localPosition.z);
